const { Queue, Registration, Patient, Doctor, Polyclinic, sequelize } = require('../models');
...
async function callQueue(req, res, next) {
  ...
  try {
    ...
    await queue.update({ status: 'Dipanggil', calledAt: new Date() }, { transaction: t });

    // Sinkronisasi: begitu pasien dipanggil ke ruang periksa, status registration ikut berubah
    await queue.registration.update({ status: 'Pemeriksaan' }, { transaction: t });
...
async function updateStatus(req, res, next) {
  ...
  try {
    ...
    if (status === 'Selesai') {
      await queue.registration.update({ status: 'Selesai' }, { transaction: t });
    }
